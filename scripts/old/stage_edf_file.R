...
# --------------------------------------------------------------------------
stations <- list.dirs(
  file.path(getwd(), "data", device, "raw"),
  full.names = F,
  recursive = F
)
...
# --------------------------------------------------------------------------
result <- foreach(
    file_name = edf_file_paths,
    .options.snow = list(progress=prop_progress)
  ) %dopar% {
  # convert from edf format to list
  edf_file <- edf::read.edf(
    filename = file.path(edf_dir, file_name),
    read.annotations = F
  )
  ...
  dt_signal <- strptime(
    paste(header_global[1, 4], " ", header_global[1, 5], sep = ""),
    "%d.%m.%y %H.%M.%S"
  )
