...
with mlflow.start_run():
    ...
    mlflow.log_param('kld_annealing_start_epoch', kld_annealing_start_epoch)
    mlflow.log_param('kld_annealing_max', kld_annealing_max)
    mlflow.log_param('kld_annealing_intervals', kld_annealing_intervals)
    mlflow.log_param('kld_latent_loss_weight',  kld_latent_loss_weight)
    mlflow.log_param('kld_attention_loss_weight',  kld_attention_loss_weight)
    ...
    for epoch in range(n_epochs):
        ...
        mlflow.log_metric('KLD-Annealing weight', kld_error_weight, step=epoch)
        mlflow.log_metric('Reconstruction Loss', (epoch_recon_loss / t), step=epoch)
        mlflow.log_metric('KLD-Latent Loss', (epoch_kld_latent_loss / t), step=epoch)
        mlflow.log_metric('KLD-Attention Loss', (epoch_kld_attention_loss / t), step=epoch)
        mlflow.log_metric('Loss', (epoch_loss / t), step=epoch)
    

    mlflow.pytorch.log_model(
        pytorch_model=smavra,
        artifact_path='model', 
        conda_env='./environment.yml', 
        code_paths=['./atemteurer']
    )
